...
```{note}
**Printable materials.** A print-ready PDF of the six station cards - each figure paired with its prompt and a blank Synthesis Card - is provided alongside this page as `lecture_30_synthesis_station_cards.pdf`. Print one card per table and one worksheet per student.
```
